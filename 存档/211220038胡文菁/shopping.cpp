      ...
      if (amount > this_stock) {
        ...
        for(int m = 0; m < len; m++){
          if(store_cart[m].user_id == user.id){
            ...
            cout << "\033[01;32m加入成功！\033[0m" << endl;
            cout << "******************************" << endl;
...
void Cart::rm_cart(User &user) {
  ...
  if(res){
    cout << "\033[01;33m确认要将该商品移出购物车吗？\033[0m" << endl;
    cout << "******************************" << endl;
    cout << "商品ID：" << this_good_id << endl;
    cout << "商品名称：" << store_goods[i].good_name << endl;
    cout << "商品价格：" << store_goods[i].good_price << endl;
    ...
    cout << "商品描述：" << store_goods[i].information << endl;
    cout << "******************************" << endl;
    cout << "\033[01;33m请选择是否移出购物车(y/n)\033[0m";
    char opt;
    cin >> opt;
    while(!cin || getchar()!='\n' || (opt !='y' && opt != 'n')){
    ...
    if(opt == 'n'){
      cout << "\033[01;33m取消移出\033[0m" << endl;
    }
    else{
      cout << "\033[01;32m成功移出！\033[0m" << endl;
...
void Cart::buy_all(User &user, Buyer &buyer) {
  ...
  if(alive_amount == 0){
    cout << "\033[01;31m您的购物车中尚无正在销售的商品！\033[0m" << endl;
    cout << endl;
    return ;
  }
  ...
  cout << "\033[01;33m总金额为：" << money_amount << "\033[0m" << endl;
  cout << "\033[01;33m友情提示：购买后会自动删除购物车中商品！\033[0m" << endl;
  cout << "\033[01;33m请选择是否全部购买(y/n)\033[0m";
  char opt;
  cin >> opt;
  while(!cin || getchar()!='\n' || (opt !='y' && opt != 'n')){
    cout <<"\033[01;31m输入不合法！请重新输入：\033[0m";
  ...
  if(opt == 'n'){
    cout << "\033[01;33m取消购买\033[0m" << endl;
  }
  else {
    if(all_money > stod(user.money)){
      cout << "\033[01;31m您的余额不足！\033[0m" << endl;
    }
    else{
      cout << "\033[01;32m购买成功！\033[0m" << endl;
      for(int i = 0; i < len; i++){
        if(store_cart[i].user_id == user.id){
          int cart_len = store_cart[i].shoppingCart.size();
          for(int j = 0; j < cart_len; j++){
